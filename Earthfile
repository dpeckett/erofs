...
package:
  ...
  RUN mkdir -p /workspace/golang-github-dpeckett-erofs
  WORKDIR /workspace/golang-github-dpeckett-erofs
  COPY . .
  ...
  RUN VERSION=$(git describe --tags --abbrev=0 | tr -d 'v') \
    && tar -czf ../golang-github-dpeckett-erofs_${VERSION}.orig.tar.gz .
